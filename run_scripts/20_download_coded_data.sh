...
if [ $# -ne 3 ]; then
    echo "Usage: sh 20_download_coded_data.sh <data-root> <coda-root> <coda-crypto-token>"
...
do
    ...
    pipenv run python get.py "$CRYPTO_TOKEN" "$DATASET" messages > "$DATA_DIR/20 Data for WS Migration/${DATASET}.json"
done
